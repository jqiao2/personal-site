...
async function main() {
  ...
  const eat = (x, y) => { if (x < minX) minX = x;
 ...
 for (const k in baseNbr) nbr[k] = new Set(baseNbr[k]);
      ...
      for (const ek in onEdge) {
        ...
        if (nbr[a]) nbr[a].delete(b);

 if (nbr[b]) nbr[b].delete(a);
        let prev = a;
    ...
    for (const svc in bySvc) {
      for (let iter = 0; iter < 500; iter++) {
        ...
        if (mid.length === 1) rm = es.find((e) => e.x !== mid[0] && e.y !== mid[0]);
          ...
          const dr = (e) => { const t = tri.find((v) => v !== e.x && v !== e.y);

 return (D(e.x, t) + D(t, e.y)) / (e.d || 1); };
  ...
  //     specific spots that diverge from the official diagram (the source of truth). ---
  {
    ...
    function removeSeg(svc, nameA, nameB) {
      ...
      if (!A || !B) { console.warn('  correction removeSeg: station not found', svc, nameA, nameB);
 ...
 return; }
    ...
    function addSeg(svc, nameA, nameB, pts) {
      ...
      if (!A || !B) { console.warn('  correction addSeg: station not found', svc, nameA, nameB);
 ...
 return; }
    ...
    function setSegPts(svc, nameA, nameB, pts) {
      ...
      if (!seg) { console.warn('  correction setSegPts: no segment', svc, nameA, nameB);
 ...
 return; }
    ...
    function copySegPts(fromSvc, toSvc, nameA, nameB) {
      ...
      if (!src || !dst) { console.warn('  correction copySegPts: missing', fromSvc, toSvc, nameA, nameB);
 ...
 while (da < -Math.PI) da += 2 * Math.PI;
    ...
    // right angles — the global corner-rounding below gives them the house radius.
    {
      ...
      for (const nm of ['3 Av-138 St', 'Brook Av', 'Cypress Av']) { const s = findSt(nm, '6');
 ...
 if (s) s.y = yRow; }
  ...
  const ptSeg = (p, a, b) => { const dx = b[0] - a[0], dy = b[1] - a[1], l2 = dx * dx + dy * dy || 1e-9; let t = ((p[0] - a[0]) * dx + (p[1] - a[1]) * dy) / l2; t = Math.max(0, Math.min(1, t));

 return Math.hypot(p[0] - (a[0] + t * dx), p[1] - (a[1] + t * dy)); };

  const ptToPath = (p, pts) => { if (pts.length === 1) return Math.hypot(p[0] - pts[0][0], p[1] - pts[0][1]); let m = Infinity;

 for (let i = 0; i < pts.length - 1; i++) { const d = ptSeg(p, pts[i], pts[i + 1]);
 ...
 return m; };
  ...
  //     only flip sides to match the official diagram. ---
  {
    ...
    function flipOffset(svc, nameA, nameB) {
      ...
      if (!seg) { console.warn('  offset flip: no segment', svc, nameA, nameB);
 ...
 return; }
    ...
    function setOffset(svc, nameA, nameB, o) {
      ...
      if (!seg) { console.warn('  offset set: no segment', svc, nameA, nameB);
