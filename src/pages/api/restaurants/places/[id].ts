	...
	try {
		...
		if (typeof body.name === 'string' || hasPlaceEdit(body)) {
			...
			await updatePlace(id, {
				...
				...('houseNumber' in body ? { houseNumber: text(body.houseNumber) } : {}),
				...('road' in body ? { road: text(body.road) } : {}),
				...('neighborhood' in body ? { neighborhood: text(body.neighborhood) } : {}),
				...('quarter' in body ? { quarter: text(body.quarter) } : {}),
				...('borough' in body ? { borough: text(body.borough) } : {}),
				...
				...('osmType' in body ? { osmType: text(body.osmType) } : {}),
				...('osmId' in body ? { osmId: num(body.osmId) } : {}),
				...('placeRank' in body ? { placeRank: num(body.placeRank) } : {}),
