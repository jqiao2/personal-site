...
import { listFavorites, setFavorite, reorderFavorites, FavoritesFullError } from '../../../lib/films';
...
// PUT    /api/films/favorites { order:[...] }→ (owner) persist a new display order.
...
export const PUT: APIRoute = async ({ request, cookies }) => {
	...
	if (!Array.isArray(body.order) || body.order.some((v) => !Number.isInteger(Number(v)))) {
		return apiError('order must be an array of tmdb ids', 400);
	}

	try {
		await reorderFavorites(body.order.map(Number));
		return json({ ok: true });
	} catch (e) {
		return apiError(e instanceof Error ? e.message : 'failed to reorder favorites', 500);
	}
};
